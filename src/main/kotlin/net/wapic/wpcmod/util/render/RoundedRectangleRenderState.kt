...
data class RoundedRectangleRenderState(
	...
	override fun setupVertices(vertices: VertexConsumer, depth: Float) {
		...
		consumer.vertex(matrix, x, y + height, depth).texture(0f, 0f).texture(width, height).texture(radius, radius)
			.texture(radius, radius).color(color.red, color.green, color.blue, color.alpha)
		consumer.vertex(matrix, x + width, y + height, depth).texture(width, 0f).texture(width, height).texture(radius, radius)
			.texture(radius, radius).color(color.red, color.green, color.blue, color.alpha)
		consumer.vertex(matrix, x + width, y, depth).texture(width, height).texture(width, height).texture(radius, radius)
			.texture(radius, radius).color(color.red, color.green, color.blue, color.alpha)
		consumer.vertex(matrix, x, y, depth).texture(0f, height).texture(width, height).texture(radius, radius)
			.texture(radius, radius).color(color.red, color.green, color.blue, color.alpha)
	}
