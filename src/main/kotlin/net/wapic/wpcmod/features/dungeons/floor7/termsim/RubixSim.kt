...
object RubixSim : TermSimGUI(
    ...
    private val panes = listOf(Items.STAINED_GLASS_PANE.orange, Items.STAINED_GLASS_PANE.yellow, Items.STAINED_GLASS_PANE.green, Items.STAINED_GLASS_PANE.blue, Items.STAINED_GLASS_PANE.red)
    private val indices = listOf(12, 13, 14, 21, 22, 23, 30, 31, 32)

    override fun create() {
        createNewGui {
            if (floor(it.containerSlot / 9f) in 1f..3f && it.containerSlot % 9 in 3..5) getPane()
            else blackPane
        }
    }
    ...
    override fun slotClick(slot: Slot, button: Int) {
		val current =
			order.find { it == ((slot.item.item as? BlockItem)?.block as? StainedGlassPaneBlock)?.color } ?: return
        createNewGui {
            if (it == slot) {
